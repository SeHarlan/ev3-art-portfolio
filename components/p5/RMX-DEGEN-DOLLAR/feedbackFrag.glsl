...
void main() {
  ...
  float chunk = u_resolution.x/(6. * chunkMult);
  ...
  bool useSweep = snoise(noiseBlock + timeBlock) < 0.2;
  bool useReset = snoise(noiseBlock + timeBlock + 100.0) < 0.5;

  float rate = .02;
  if(useSweep ) {
    float xOff = rate * (noiseNegNeutralPos(noiseBlock + timeBlock + 200.));
    float yOff = rate;
