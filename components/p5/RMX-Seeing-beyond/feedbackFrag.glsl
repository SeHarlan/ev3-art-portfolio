...
void main() {
  ...
  float chunkMult = 1.0 + random(timeBlock + 100.0) * 3.;
  
  float chunk = u_resolution.x/(4. * chunkMult);
  ...
  bool useSweep = snoise(noiseBlock + timeBlock) < 0.2;//< (1.0 - posBlockFloor.y * blockSize.y) * 0.4;
  bool useReset = snoise(noiseBlock + timeBlock + 100.0) < 0.5;

  float rate = .01;
  if(useSweep ) {
    float xOff = rate * abs(noiseNegNeutralPos(noiseBlock + timeBlock + 200.));
    float yOff = rate;
