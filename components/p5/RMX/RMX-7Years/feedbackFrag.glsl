...
void main() {
  ...
  if(noise(posBlockFloor * 0.075 + u_time*0.1)<0.5) {
    vec4 currentCheck = texture2D(u_texture, posBlockFloor * blockSize);
    float currentBrightness = (currentCheck.r + currentCheck.g + currentCheck.b) / 3.0;

    if (currentBrightness < 0.4) {
    ...
    gl_FragColor = texture2D(u_originalImage, st);
    return;
  }

  vec2 blockSt = (posBlockFloor + posBlockOffset) * blockSize;

  vec4 color = texture2D(u_texture, blockSt);
