...
void main() {
  ...
  //sstatic
  float staticLineTime = fract(u_time * 0.1) * 100.5;
  float staticFuzzTime = fract(u_time * .003);
  vec2 staticSt = floor(orgSt*u_resolution);
  vec2 staticLines = vec2(staticSt.x * 100.- staticLineTime , staticSt.y * 20. ) ;
  vec2 staticFuzz = floor(st * u_resolution * 0.66) - staticFuzzTime;
  float tvRan = mix(random(staticLines) * randomNegPos(staticLines) , random(staticFuzz),0.25);
