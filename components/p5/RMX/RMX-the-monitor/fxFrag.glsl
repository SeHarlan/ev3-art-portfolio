...
void main() {
  ...
  float tvRan = mix(random(staticLines) * randomNegPos(staticLines) , random(staticFuzz) * randomNegPos(staticFuzz), 0.2);
  ...
  color.gr -= .2 + 0.2 * sin(u_time * 3. + (st.y + 1.0-st.x) * PI * 4. );
